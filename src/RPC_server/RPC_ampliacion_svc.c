...
login_1(struct svc_req *rqstp, register SVCXPRT *transp)
{
	...
	if (!login_1_freeresult (transp, _xdr_result, (caddr_t) &result))
...
main (int argc, char **argv)
{
	...
	if (!svc_register(transp, LOGIN, LOGINVER1, login_1, IPPROTO_UDP)) {
		fprintf (stderr, "%s", "unable to register (LOGIN, LOGINVER1, udp).");
		exit(1);
	}
	...
	if (!svc_register(transp, LOGIN, LOGINVER1, login_1, IPPROTO_TCP)) {
		fprintf (stderr, "%s", "unable to register (LOGIN, LOGINVER1, tcp).");
		exit(1);
	}
